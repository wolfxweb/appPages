...
<head>
    ...
    <link rel="shortcut icon" href="{{ asset('assets/front/img/user/vcard/' . $vcard->profile_image) }}"
        type="image/x-icon">
    <!--====== Bootstrap css ======-->
    ...
    <script>
        ...
        fbq('init', {{ $vcard->pixel_facebook }});
        fbq('track', 'PageView');
    </script>
    <noscript>
        <img height="1" width="1" style="display:none"
            src="https://www.facebook.com/tr?id={{ $vcard->pixel_facebook }}&ev=PageView&noscript=1" />
    </noscript>
    <!-- Fim do código do pixel do Facebook -->
    <!-- Código de rastreamento do Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id={{ $vcard->google_analitics }}"></script>
    <script>
        ...
        gtag('config', {{ $vcard->google_analitics }});
    </script>
    <script>
        @php
            // echo htmlspecialchars_decode($vcard->pixel_facebook);
            //  echo htmlspecialchars_decode($vcard->google_analitics);
        @endphp
    </script>
</head>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">{{ $keywords['Share_On'] ?? 'Compartilhar' }}
                    </h5>
                ...
                <div class="modal-body">
                    <div class="actions">
                        ...
                        <div class="action-btn">
                            <a class="twitter" href="https://twitter.com/intent/tweet?text={{ url()->current() }}"><i
                                    class="fab fa-twitter"></i></a>
                            <br>
                            <span>{{ $keywords['Twitter'] ?? 'Twitter' }}</span>
                        </div>
                        <div class="action-btn">
                            <a class="whatsapp" href="whatsapp://send?text={{ url()->current() }}"><i
