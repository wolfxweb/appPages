...
@section('content')
        ...
        @if (is_null($package))
            ...
            @if ($pendingPackage)
                ...
                <div class="alert alert-warning">
                    <strong>Plano pendente: </strong> {{$pendingPackage->title}} 
            ...
            <div class="col-12">
                <div class="alert border-left border-primary text-dark">
                    ...
                    @if ($current_membership->is_trial == 1)
                        (Data expiração: {{Carbon\Carbon::parse($current_membership->expire_date)->format('M-d-Y')}})
                        <span class="badge badge-primary">Trial</span>
                    @else
                        (Data expiração: {{$current_package->term === 'lifetime' ? "Lifetime" : Carbon\Carbon::parse($current_membership->expire_date)->format('M-d-Y')}})
                    @endif
            ...
            <div class="col-md-3 pr-md-0 mb-5">
                <div class="card-pricing2 @if(isset($current_package->id) && $current_package->id === $package->id) card-success @else card-primary @endif">
                    <div class="pricing-header">
                        ...
                        <h3 class="badge badge-danger d-inline-block float-right ml-2">Atual</h3>
                        @endif
                        @if($package_count >= 2 && $next_package->id == $package->id)
                            @if ($next_membership->status == 1)
                            <h3 class="badge badge-warning d-inline-block float-right ml-2">Proximo</h3>
                            @endif
                        @endif
                    ...
                    @if($package_count < 2 && !$hasPendingMemb)
                        <div class="px-4">
                            @if(isset($current_package->id) && $current_package->id === $package->id)
                                ...
                                class="btn btn-success btn-lg w-75 fw-bold mb-3">{{__('Estender')}}</a>
                                @endif
                            @else
                                <a href="{{route('user.plan.extend.checkout',$package->id)}}"
                                class="btn btn-primary btn-block btn-lg fw-bold mb-3">{{__('Comprar')}}</a>
                            @endif
                        </div>
                    @endif
                </div>
