<div class="sidebar-widget-area">
    <div class="widget search-widget mb-40">
        ...
        <form action="{{route('front.user.blogs', getParam())}}">
    ...
    <div class="widget categories-widget mb-40">
        ...
        <ul class="widget-link">
            <li @if(empty(request()->input('category'))) class="active" @endif><a href="{{route('front.user.blogs', getParam())}}">{{$keywords["All"] ?? "All"}} <span>({{$allCount}})</span></a></li>
            @foreach ($blog_categories as $bc)
            <li @if($bc->id == request()->input('category')) class="active" @endif><a href="{{route('front.user.blogs', getParam()) . '?category=' . $bc->id}}">{{$bc->name}} <span>({{$bc->blogs()->count()}})</span></a></li>
            @endforeach
        </ul>
    </div>
            ...
            @foreach ($latestBlogs as $lb)
                <li class="post-thumbnail-content">
                    ...
                    <div class="post-title-date">
                        <h6><a href="{{route('front.user.blog.detail', [getParam(), $lb->slug, $lb->id])}}">{{strlen($lb->title) > 30 ? mb_substr($lb->title, 0, 30, 'UTF-8') . '...' : $lb->title}}</a></h6>
